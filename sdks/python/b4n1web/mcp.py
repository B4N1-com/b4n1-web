...
class McpClient:
    ...
    def type_text(self, selector: str, text: str, clear_first: bool = False) -> str:
    ...
    def wait_for_selector(self, selector: str, timeout_ms: int = 5000) -> str:
    ...
    def screenshot(self, url: str, full_page: bool = False) -> str:
        """Take a screenshot of a page.

        Args:
            url: URL to screenshot
            full_page: Capture full scrollable page (render mode only)

        Returns:
            Base64-encoded PNG data URI
        """
        response = self._call_tool("screenshot", {"url": url, "full_page": full_page})
        return self._get_text(response)

    async def screenshot_async(self, url: str, full_page: bool = False) -> str:
        ...
        """
        response = await self._call_tool_async(
            "screenshot", {"url": url, "full_page": full_page}
        )
        return self._get_text(response)
    ...
    async def iframe_click_async(self, frame_index: int, selector: str) -> str:
    ...
    def set_viewport(self, width: int, height: int) -> str:
        ...
        """
        response = self._call_tool("set_viewport", {"width": width, "height": height})
        return self._get_text(response)
    ...
    async def set_user_agent_async(self, ua: str) -> str:
        ...
        Args:
            ua: User agent string
        """
        response = await self._call_tool_async("set_user_agent", {"ua": ua})
        return self._get_text(response)
    ...
    async def emulate_device_async(self, device: str) -> str:
        ...
        Args:
            device: Device name (e.g. "iPhone 12", "desktop")
        """
        response = await self._call_tool_async("emulate_device", {"device": device})
        return self._get_text(response)
    ...
    def upload_file(self, selector: str, path: str) -> str:
        ...
        """
        response = self._call_tool("upload_file", {"selector": selector, "path": path})
        return self._get_text(response)
    ...
    def download_file(self, url: str, output: str) -> str:
        ...
        """
        response = self._call_tool("download_file", {"url": url, "output": output})
        return self._get_text(response)
    ...
    def get_links_from_page(self, url: str) -> List[str]:
        ...
        """
        response = self._call_tool("get_links_from_page", {"url": url})
        text = self._get_text(response)
        try:
            return json.loads(text.strip())
        except Exception:
    ...
    async def get_links_from_page_async(self, url: str) -> List[str]:
        ...
        Args:
            url: URL to extract links from
        """
        response = await self._call_tool_async("get_links_from_page", {"url": url})
        text = self._get_text(response)
        try:
            return json.loads(text.strip())
        except Exception:
...
class AsyncMcpClient:
    ...
    async def wait_for_selector_async(
        ...
        return await self.client.wait_for_selector_async(selector, timeout_ms)
